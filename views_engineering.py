...
# Import defensivo — se o módulo SDS falhar (dependência ausente, etc.),
# o restante do módulo de Engenharia continua funcionando normalmente.
try:
    from views_sds import render_sds_upload_panel
    _SDS_AVAILABLE = True
except Exception:
    _SDS_AVAILABLE = False


def render_engineering_module(profile, menu_styles: dict):

    # Monta a lista de tabs dinamicamente
    if _SDS_AVAILABLE:
        tab_options = ["Leitor SDS/FISPQ", "Termodinâmica", "Inertização (NFPA 69)", "Emergências (PSV/Runaway)"]
        tab_icons = ["file-earmark-text", "thermometer", "cone-striped", "speedometer2"]
    else:
        tab_options = ["Termodinâmica", "Inertização (NFPA 69)", "Emergências (PSV/Runaway)"]
        tab_icons = ["thermometer", "cone-striped", "speedometer2"]
    ...
    if eng_tab == "Leitor SDS/FISPQ" and _SDS_AVAILABLE:
