...
from data.chemicals import DB
from data.refs import REFS
from data.hazop_db import HAZOP_DB
from services.ai_client import AIClient
from services.deterministic import IPL_CATALOG, chemical_lookup, compute_lopa, gaussian_dispersion, hazop_template, pool_fire, recommend_modules
from services.hazard_extractor import extract_document_insights, generate_hazop_from_text
from services.rag import LocalKnowledgeBase
from services.report_service import ReportService
from ui.theme import APP_CSS

st.set_page_config(page_title=settings.app_name, page_icon='⚗️', layout='wide', initial_sidebar_state='expanded')
...
overview, chemicals_tab, hazop_tab, bowtie_tab, lopa_tab, consequence_tab, docs_tab, copilot_tab, reports_tab, refs_tab = st.tabs([
    'Overview', 'Consulta química', 'HAZOP + IA', 'Bow-Tie', 'LOPA → SIL', 'Consequências', 'Document Intelligence', 'Copiloto', 'Relatórios', 'Referências'
])

with overview:
    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(metric_card('OpenAI', 'ON' if ai.enabled else 'OFF', 'risk-green' if ai.enabled else 'risk-amber'), unsafe_allow_html=True)
    c2.markdown(metric_card('Chunks RAG', str(len(kb.chunks))), unsafe_allow_html=True)
    c3.markdown(metric_card('Cenários HAZOP IA', str(len(st.session_state.get('risk_register', [])))), unsafe_allow_html=True)
    c4.markdown(metric_card('Último caso', st.session_state.get('last_case_name', 'untitled')), unsafe_allow_html=True)
    st.markdown("<div class='panel'><div class='note-card'>Arquitetura recomendada: use a IA para estruturar, comparar, resumir e redigir. Mantenha decisões numéricas críticas no motor determinístico e sob revisão de engenharia.</div></div>", unsafe_allow_html=True)
...
with chemicals_tab:
    ...
    if find_btn and q:
        ...
        if result:
            ...
            st.session_state.history.append({'name': result['nome'], 'at': datetime.now().strftime('%H:%M')})
        else:
    ...
    if compound:
        ...
        d.markdown(metric_card('AIT', f"{compound.get('ait', '—')} °C" if compound.get('ait') else '—'), unsafe_allow_html=True)
        l1, l2 = st.columns(2)
        with l1:
        ...
        st.markdown("<div class='small-muted'>Use esta base como screening rápido. Para classificação formal, confira a SDS/FISPQ do fornecedor e critérios regulatórios vigentes.</div>", unsafe_allow_html=True)
    else:
        st.info('Selecione um composto na barra lateral ou pesquise por nome/CAS.')

with hazop_tab:
    c1, c2, c3 = st.columns(3)
    equipment = c1.selectbox('Equipamento / nó', ['Tanque atmosférico', 'Reator CSTR exotérmico', 'Vaso de pressão', 'Trocador de calor', 'Tubulação de processo', 'Bomba centrífuga'])
    ...
    process_description = st.text_area('Descrição do processo', height=180, placeholder='Descreva o processo, os fluidos, utilidades, instrumentos e condição operacional.')
    operating_context = st.text_area('Contexto operacional', height=100, placeholder='Ex.: resfriamento por água industrial, atmosfera inerte, operação em batelada...')
...
with bowtie_tab:
    st.markdown("<div class='panel'><b>Bow-Tie simplificado</b><div class='small-muted'>Use esta aba como estrutura visual de ameaças → top event → consequências.</div></div>", unsafe_allow_html=True)
    ...
    prev = st.text_area('Barreiras preventivas', value='Inspeção de integridade\nLAHH + SIS\nProcedimento operacional\nBarreiras físicas')
    cons = st.text_area('Consequências', value='Pool fire\nVCE\nContaminação ambiental\nPerda econômica')
    mitig = st.text_area('Barreiras mitigadoras', value='Dique\nDetecção de incêndio\nEspuma\nPlano de emergência')
...
with lopa_tab:
    ...
    with right:
        selected = st.multiselect('IPLs independentes', options=[f'{n} (PFD={p})' for n, p in IPL_CATALOG], default=[f'{IPL_CATALOG[0][0]} (PFD={IPL_CATALOG[0][1]})'])
    ...
    if st.session_state.lopa_result:
        ...
        c.markdown(metric_card('MCF', f"{r['mcf']:.2e}/ano", 'risk-green' if r['ratio'] <= 1 else 'risk-red'), unsafe_allow_html=True)
        d.markdown(metric_card('SIL requerido', r['sil'], 'risk-green' if r['sil'] == 'Não requerido' else 'risk-amber'), unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(r['selected_ipls'], columns=['IPL', 'PFD']), use_container_width=True, hide_index=True)
        st.markdown(f"<div class='note-card'>Razão MCF/critério = <b>{r['ratio']:.2f}</b>. {'Risco tolerável' if r['ratio'] <= 1 else 'Adicionar camada independente ou reduzir frequência do evento iniciador.'}</div>", unsafe_allow_html=True)
...
with consequence_tab:
    ...
    with disp_tab:
        ...
        if st.session_state.dispersion_result:
            ...
            u1, u2 = st.columns(2)
            u1.markdown(metric_card('Distância até IDLH', x_display, 'risk-red' if r['x_idlh'] and r['x_idlh'] < 500 else 'risk-green'), unsafe_allow_html=True)
    ...
    with fire_tab:
        ...
        if st.session_state.pool_fire_result:
            ...
            p3.markdown(metric_card('Fluxo no alvo', f"{r['q_kW_m2']:.2f} kW/m²", 'risk-red' if r['q_kW_m2'] > 12.5 else 'risk-amber' if r['q_kW_m2'] > 4.7 else 'risk-green'), unsafe_allow_html=True)
    ...
    with suggest_tab:
        ...
        compound_name = st.text_input('Composto principal', value=st.session_state.selected_compound['nome'] if st.session_state.selected_compound else '')
...
with docs_tab:
    uploads = st.file_uploader('Enviar documentos técnicos', accept_multiple_files=True, type=['pdf', 'docx', 'xlsx', 'xls', 'csv', 'txt', 'md', 'json'])
    ...
    search_query = st.text_input('Buscar no acervo da sessão', placeholder='Ex.: LAHH, amônia, intertravamento, temperatura máxima')
    if search_query:
        hits = kb.search(search_query, top_k=6)
        for hit in hits:
            st.markdown(f"**{hit.chunk.source_name}** · página {hit.chunk.page or '—'} · score={hit.score:.3f} · {hit.match_reason}")
...
with copilot_tab:
    st.markdown("<div class='panel'><b>Copiloto técnico</b><div class='small-muted'>Pergunte sobre HAZOP, salvaguardas, lacunas, cenários de consequência ou síntese documental.</div></div>", unsafe_allow_html=True)
    ...
    if prompt := st.chat_input('Ex.: Quais lacunas de salvaguarda aparecem neste caso?'):
        ...
        context_blocks.append({'source': 'session_state', 'text': json.dumps(structured_context, ensure_ascii=False, default=str)[:7000]})
...
with refs_tab:
    ...
    st.markdown("<div class='small-muted'>Esta base referencia o núcleo determinístico e a linguagem dos relatórios. Para uso formal, valide edição, critérios corporativos e aderência regulatória local.</div>", unsafe_allow_html=True)
