...
def render_flammability_envelope(lfl, ufl, loc):
    ...
    # Marcador de Segurança (Abaixo do LOC)
    safe_margin = loc * 0.6  # 60% do LOC é uma margem de segurança típica
    fig.add_trace(go.Scatter(x=[safe_margin], y=[lfl/2], mode='markers+text', marker=dict(color='#10b981', size=12),
...
# ==============================================================================
    ...
    if e_tab == "Termodinâmica":
        ...
        st.markdown("<div class='note-card'>Calcule a atmosfera segura durante partidas e paradas. Para evitar a mistura explosiva, a concentração de O₂ deve operar abaixo da <b>Limiting Oxygen Concentration (LOC)</b> com margem de segurança.</div>", unsafe_allow_html=True)
        ...
        with c1:
            ...
            safe_o2 = loc * 0.6 # Exemplo de regra prática da NFPA (60% do LOC)
            st.metric("Margem de Segurança (Alarme Alto)", f"O₂ < {safe_o2:.1f}%")
        ...
        st.markdown("<div class='note-card'>Utilize os botões ⚙️ para expandir os parâmetros de cálculo apenas quando necessário. A interface foca nos resultados críticos.</div>", unsafe_allow_html=True)
