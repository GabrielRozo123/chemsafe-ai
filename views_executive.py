...
def render_executive_module(
    ...
    if exec_tab == "Dashboard Global":
        render_hero_panel(
            ...
            subtitle=(
                "Visão consolidada da prontidão do caso, gaps críticos, workflow e ações abertas "
        ...
        if st.session_state.audit_mode:
            render_evidence_panel(
                ...
                assumptions=[
                    "Indicadores são de priorização e screening executivo.",
                    "A decisão formal requer revisão técnica complementar.",
                    "A robustez do caso depende da qualidade dos dados e do contexto operacional.",
                ],
                inputs={
                    "Maturidade global": f"{cri_data.get('index', 0)}%",
                    "PSI score": f"{float((psi_summary or {}).get('score', 0) or 0):.1f}",
                    "Gate": st.session_state.case_decision_gate,
        ...
        if st.session_state.audit_mode:
            render_evidence_panel(
                ...
                assumptions=[
                    "Estimativa conceitual preliminar.",
                    "Classificação depende da semântica do texto das ações.",
                    "Faixas precisam recalibração por escopo real.",
                ],
                inputs={
        ...
        if has_actions:
            ...
            if "Hierarquia NIOSH" in action_df_dash.columns:
                col_config["Hierarquia NIOSH"] = st.column_config.TextColumn("Hierarquia (Auto)", width="medium")
            if "Pacote AACE" in action_df_dash.columns:
                col_config["Pacote AACE"] = st.column_config.TextColumn("Pacote AACE", width="medium")
            if "Custo Min (R$)" in action_df_dash.columns:
                col_config["Custo Min (R$)"] = st.column_config.NumberColumn("Min", format="R$ %.0f")
            if "Custo P50 (R$)" in action_df_dash.columns:
                col_config["Custo P50 (R$)"] = st.column_config.NumberColumn("P50", format="R$ %.0f")
            if "Custo Máx (R$)" in action_df_dash.columns:
                col_config["Custo Máx (R$)"] = st.column_config.NumberColumn("Máx", format="R$ %.0f")
        ...
        render_hero_panel(
            ...
            subtitle="Salve e recupere estudos em andamento para manter continuidade analítica, workflow e rastreabilidade do trabalho.",
        ...
        with col_save:
            ...
            if st.button("Salvar Progresso", type="primary"):
                ...
                _ensure_review_history(
                    case_status=st.session_state.case_status,
                    case_gate=st.session_state.case_decision_gate,
                    actor=st.session_state.case_owner or "analista",
                    note=st.session_state.case_status_note,
                )
                save_case(
                    ...
                    pssr_result=st.session_state.get("pssr_result"),
                    reactivity_result=st.session_state.get("reactivity_result"),
                    current_node_name=st.session_state.current_node_name,
                    case_status=st.session_state.case_status,
                    case_status_note=st.session_state.case_status_note,
                    case_owner=st.session_state.case_owner,
                    case_reviewer=st.session_state.case_reviewer,
                    case_decision_gate=st.session_state.case_decision_gate,
        ...
        if cases:
            ...
            render_empty_state(
                ...
                message="Salve o caso atual para habilitar rastreabilidade, retomada de estudos e comparação entre cenários.",
            )

    elif exec_tab == "Governança do Caso":
        render_hero_panel(
            title="Governança, Workflow e Rastreabilidade",
            subtitle="Consolide status do caso, gate decisório, histórico de revisão e evidências por cálculo para tornar o pacote defensável.",
            kicker="Case Governance",
        )
        ...
        with left:
            ...
            st.session_state.case_status = st.selectbox(
                "Status do caso",
                CASE_STATUS_OPTIONS,
                index=CASE_STATUS_OPTIONS.index(st.session_state.case_status) if st.session_state.case_status in CASE_STATUS_OPTIONS else 0,
            )
            suggested_gate = infer_case_gate(
                cri_data=cri_data,
                psi_summary=psi_summary or {},
                gaps_criticos=gaps_criticos,
                moc_result=st.session_state.get("moc_result"),
            ...
            st.session_state.case_status_note = st.text_area(
                "Nota técnica de status",
                value=st.session_state.case_status_note,
                height=140,
                placeholder="Explique por que o caso está neste status, o que bloqueia avanço e o que precisa ser fechado.",
            )
        ...
        with right:
            ...
            payload = build_case_snapshot_payload(
                ...
                case_status=st.session_state.case_status,
                case_gate=st.session_state.case_decision_gate,
                status_note=st.session_state.case_status_note,
                review_history=st.session_state.get("review_history", []),
                traceability_df=traceability_df,
                action_df=action_df_dash,
            )
            ...
            with cjson:
                st.download_button(
                    ...
                    file_name=f"{_safe_case_name(st.session_state.current_case_name, profile)}_snapshot.json",
            ...
            with chtml:
                st.download_button(
                    ...
                    snapshot_html,
                    file_name=f"{_safe_case_name(st.session_state.current_case_name, profile)}_snapshot.html",
                    mime="text/html",
                    use_container_width=True,
                )
            ...
            if traceability_df is not None and not getattr(traceability_df, "empty", True):
                ...
                render_empty_state(
                    title="Sem rastreabilidade compilada",
                    message="Ainda não foi possível compilar linhas de rastreabilidade para este caso.",
                    icon="🧭",
                )
