...
from data.chemicals import resolve_compound
...
def gaussian_dispersion(q_g_s: float, wind_m_s: float, stability: str, idlh_ppm: float, molecular_weight: float, stack_height_m: float = 0.0, x_max: int = 3000, step: int = 25) -> Dict:
    ...
    for x in xs:
        sy = pg['a'] * (x ** pg['b'])
        sz = pg['c'] * (x ** pg['d'])
        c_val = q_g_s / (math.pi * sy * sz * wind_m_s) * math.exp(-((stack_height_m ** 2) / (2 * sz ** 2)))
        cs.append(c_val)
...
def pool_fire(pool_diameter_m: float, burn_rate_kg_m2_s: float, heat_of_combustion_kj_kg: float, receptor_distance_m: float) -> Dict:
    ...
    try:
        form_factor = max(0.0001, (1 / (math.pi * s)) * (
            math.atan(math.sqrt((s + 1) / (s - 1)))
            - (1 / a_v) * math.atan(math.sqrt((s + 1) / (a_v * (s - 1))))
            + (2 * hf / pool_diameter_m / b_v) * math.atan(math.sqrt((s + 1) / (b_v * (s - 1))))
        ))
    except Exception:
        form_factor = 0.01
...
def recommend_modules(process_text: str, compound_name: str = '') -> List[str]:
    ...
    rec = []
    if any(key in txt for key in ['toxico', 'toxic', 'amônia', 'ammonia', 'chlor', 'cloro', 'release', 'vazamento gasoso']):
        rec.append('Dispersão gaussiana / toxic release screening')
    if any(key in txt for key in ['inflam', 'etanol', 'solvente', 'tolueno', 'pool', 'poça', 'derramamento']):
        rec.append('Pool fire / radiação térmica')
    if any(key in txt for key in ['reator', 'exotérm', 'runaway', 'cstr', 'batch', 'pressão', 'vaso']):
